...
def addAcount():
    try:
        if session.get('user'):
            ...
            cursor.callproc('sp_addAccount',
                            (_use_name, _use_description, _use_money, _use_date, _write_date, _write_name))
...
def stat():
    try:
        if session.get('user'):
            ...
            lineValue_frequency = per_month_write_frequency

            return render_template('stat.html', values=barValue_spend, Frequency_Values=lineValue_frequency,
